...
function validateAnswer(lesson, userAnswer) {
  ...
  try {
    ...
    console.error('[LearnKids /mcp] Validation error:', error);
    return {
      correct: false,
...
const handler = createMcpHandler(
  (server) => {
    console.log('[LearnKids /mcp] Setting up MCP tools...');
    ...
    server.tool(
      ...
      {},
      async () => {
        console.log('[LearnKids /mcp] Tool called: getCourses');
        await loadCourses();
    ...
    server.tool(
      ...
      async ({ courseId }) => {
        console.log('[LearnKids /mcp] Tool called: getCourse', courseId);
        await loadCourses();
    ...
    server.tool(
      ...
      async ({ courseId, lessonId }) => {
        console.log('[LearnKids /mcp] Tool called: getLesson', { courseId, lessonId });
        await loadCourses();
    ...
    server.tool(
      ...
      async ({ courseId, lessonId, answer }) => {
        console.log('[LearnKids /mcp] Tool called: checkAnswer', {
          courseId,
          lessonId,
